...
namespace GCDSearch
{
    internal delegate int GCDDelegate(int a, int b);

    public static class GСDCalculator
    {
        private static GCDDelegate gcdOfArrayNumbers;
        ...
        private static int GCDOfSeveralNumbers(int[] numbers, Stopwatch time)
        {
            ...
            int divider = gcdOfArrayNumbers(numbers[0], numbers[1]);

            for (int i = 2; i < numbers.Length; i++)
            {
                divider = gcdOfArrayNumbers(divider, numbers[i]);
            }
